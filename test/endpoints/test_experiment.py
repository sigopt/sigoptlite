...
class TestExperiment(UnitTestsBase):
  ...
  def test_experiment_missing_parameters(self, experiment_meta):
    ...
    with pytest.raises(SigOptException) as exception_info:
      self.conn.experiments().create(**experiment_meta)
    msg = 'Validation failed for LocalExperimentBuilder: Missing required json key "parameters" in'
  ...
  def test_experiment_parameters_empty_list(self, experiment_meta):
    ...
    with pytest.raises(SigOptException) as exception_info:
      self.conn.experiments().create(**experiment_meta)
    msg = "Validation failed for LocalExperimentBuilder: The length of .parameters must be greater than or equal to 1"
  ...
  def test_experiment_missing_metrics(self, experiment_meta):
    ...
    with pytest.raises(SigOptException) as exception_info:
      self.conn.experiments().create(**experiment_meta)
    msg = 'Validation failed for LocalExperimentBuilder: Missing required json key "metrics" in'
    assert exception_info.value.args[0].startswith(msg)
  ...
  def test_experiment_metrics_empty_list(self, experiment_meta):
    ...
    with pytest.raises(SigOptException) as exception_info:
      self.conn.experiments().create(**experiment_meta)
    msg = "Validation failed for LocalExperimentBuilder: The length of .metrics must be greater than or equal to 1"
    assert exception_info.value.args[0] == msg
